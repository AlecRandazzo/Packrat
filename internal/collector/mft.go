...
func parseMFTRecord0(handler handler) (mft.MasterFileTableRecord, error) {
	// Move handle pointer back to beginning of handler
	_, err := handler.Handle().Seek(0x00, 0)
	if err != nil {
		return mft.MasterFileTableRecord{}, fmt.Errorf("failed to seek back to handler offset 0x00: %w", err)
	}

	// Seek to the offset where the MFT starts. If it errors, bomb.
	_, err = handler.Handle().Seek(handler.Vbr().MftByteOffset, 0)
	if err != nil {
		return mft.MasterFileTableRecord{}, fmt.Errorf("failed to seek to mft: %w", err)
	}
	...
	if err != nil {
		return mft.MasterFileTableRecord{}, fmt.Errorf("failed to read the mft: %w", err)
	}

	// Sanity check that this is indeed an mft record
	err = mft.ValidateMftRecordBytes(buffer)
	if err != nil {
		return mft.MasterFileTableRecord{}, fmt.Errorf("invalid mft record: %w", err)
	}

	// Parse the MFT record
	var mft0 mft.MasterFileTableRecord
	mft0, err = mft.ParseMftRecord(buffer, handler.Vbr().BytesPerCluster)
	if err != nil {
		return mft.MasterFileTableRecord{}, fmt.Errorf("Handler.parseMFTRecord0() failed to parse the mft's mft record: %w", err)
	}
