...
// unresolvedDataRun is a data run that has not been resolved yet.
type unresolvedDataRun struct {
	clusterOffset    int64
	numberOfClusters int64
}

// unresolvedDataRuns is an map array of unresolvedDataRun
...
// dataRunSplit represents the split of a data run byte.
...
func getDataAttribute(input []byte, bytesPerCluster uint) (dataAttribute interface{}, err error) {
	...
	buffer, _ = byteshelper.GetValue(input, dataAttributeResidentFlagLocation)
	if buffer[0] == 0x00 {
