...
// ValidateDirectory will check an MFT record to determine if it is a directory or not.
...
func ConvertRawMFTRecordToDirectory(input []byte, bytesPerSector, bytesPerCluster uint) (UnResolvedDirectory, error) {
	...
	fnAttributes, _, _, _, _ = GetAttributes(rawAttributes, bytesPerCluster)
...
func buildUnresolvedDirectoryTree(reader io.Reader, bytesPerSector, bytesPerCluster uint) (UnresolvedDirectoryTree, error) {
	...
	for {
		...
		var directory UnResolvedDirectory
		directory, err = ConvertRawMFTRecordToDirectory(buffer, bytesPerSector, bytesPerCluster)
		if err != nil {
			continue
		}
...
// BuildDirectoryTree takes an MFT and creates a directory tree map where the keys are the mft record number of the UnResolvedDirectory.
func BuildDirectoryTree(reader io.Reader, volumeLetter string, bytesPerSector, bytesPerCluster uint) (DirectoryTree, error) {
	var directoryTree DirectoryTree
	unresolvedDirectoryTree, _ := buildUnresolvedDirectoryTree(reader, bytesPerSector, bytesPerCluster)
